...
def test_tg_extra_json():
    ...
    assert tg_extra_json('{"type": "Point","coordinates": [-118.2097812,34.0437074]}') == None
    assert tg_extra_json(
            '{"id": "A", "type": "Point","coordinates": [-118.2097812,34.0437074]}'
        ) == '{"id":"A"}'

def test_tg_geom():
    assert db.execute("select tg_geom(?)", ["POINT(1 1)"]).fetchone()[0] == None
    assert db.execute("select subtype(tg_geom(?))", ["POINT(1 1)"]).fetchone()[0] == 112
    with pytest.raises(
        sqlite3.OperationalError, match="ParseError: invalid text"
    ):
...
def test_tg_geometries_each(snapshot):
    ...
    assert execute_all(db, "select rowid, geometry, tg_to_geojson(geometry) from tg_geometries_each(?)", [COLLECTION_GEOJSON]) ==  snapshot
...
def test_tg_to_geojson():
    ...
    assert tg_to_geojson(shapely.to_wkt(LINE_A)) == '{"type":"LineString","coordinates":[[0,0],[1,1]]}'
    # TODO more tests
...
def test_tg_point_geojson():
    ...
    assert tg_point_geojson(1.111, 2.222) == '{"type":"Point","coordinates":[1.111,2.222]}'

def test_tg_point_wkb():
    tg_point_wkb = lambda *args: db.execute(
        "select tg_point_wkb(?, ?)", args
    ).fetchone()[0]
    assert tg_point_wkb(1, 2) == b"\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\xf0?\x00\x00\x00\x00\x00\x00\x00@"
    assert tg_point_wkb(1.111, 2.222) == b"\x01\x01\x00\x00\x00-\xb2\x9d\xef\xa7\xc6\xf1?-\xb2\x9d\xef\xa7\xc6\x01@"
